...
from audio import Audio, pad_mfcc
from constants import NUM_FBANKS, NUM_FRAMES
from conv_models import DeepSpeakerModel
from utils import ensures_dir, load_npy, load_pickle, train_test_sp_to_utt
...
class KerasFormatConverter:
    def __init__(self, working_dir, load_test_only=False):
        ...
        self.categorical_speakers = load_pickle(
            os.path.join(self.output_dir, "categorical_speakers.pkl")
        )
        if not load_test_only:
            self.kx_train = load_npy(
                os.path.join(self.output_dir, "kx_train.npy")
            )
            self.ky_train = load_npy(
                os.path.join(self.output_dir, "ky_train.npy")
            )
        ...
        if self.categorical_speakers is None:
            self.categorical_speakers = SparseCategoricalSpeakers(
                self.audio.speaker_ids
            )

    def persist_to_disk(self):
        with open(
            os.path.join(self.output_dir, "categorical_speakers.pkl"), "wb"
        ) as w:
    ...
    def generate_per_phase(
        self, max_length=NUM_FRAMES, num_per_speaker=3000, is_test=False
    ):
        # train OR test.
        ...
        for i, speaker_id in enumerate(
            tqdm(self.audio.speaker_ids, desc=desc)
        ):
            utterances_files = sp_to_utt[speaker_id]
            for j, utterance_file in enumerate(
                np.random.choice(
                    utterances_files, size=num_per_speaker, replace=True
                )
            ):
                self.load_into_mat(
                    utterance_file,
    ...
    def generate(self, max_length=NUM_FRAMES, counts_per_speaker=(3000, 500)):
        kx_train, ky_train = self.generate_per_phase(
            max_length, counts_per_speaker[0], is_test=False
        )
        kx_test, ky_test = self.generate_per_phase(
            max_length, counts_per_speaker[1], is_test=True
        )
    ...
    @staticmethod
    def load_into_mat(
        utterance_file, categorical_speakers, speaker_id, max_length, kx, ky, i
    ):
...
class SparseCategoricalSpeakers:
    def __init__(self, speakers_list):
        ...
        assert len(set(self.speaker_ids)) == len(
            self.speaker_ids
        )  # all unique.
...
class LazyTripletBatcher:
    def __init__(
        self, working_dir: str, max_length: int, model: DeepSpeakerModel
    ):
        self.working_dir = working_dir
        ...
        self.total_history_length = (
            self.nb_speakers * self.nb_per_speaker * self.history_length
        )  # 25,600
        ...
        self.batch_count = 0
        for _ in tqdm(
            range(self.history_length), desc="Initializing the batcher"
        ):  # init history.
    ...
    def get_batch(self, batch_size, is_test=False):
        return (
            self.get_batch_test(batch_size)
            if is_test
            else self.get_random_batch(batch_size, is_test=False)
        )
    ...
    def get_random_batch(self, batch_size, is_test=False):
        ...
        for anchor_speaker in anchor_speakers:
            negative_speaker = np.random.choice(
                list(set(speakers) - {anchor_speaker}), size=1
            )[0]
            ...
            neg_utterance = np.random.choice(
                sp_to_utt[negative_speaker], 1, replace=True
            )[0]
    ...
    def get_batch_train(self, batch_size):
        ...
        for anchor_index in anchor_indexes:
            ...
            anchor_cos = batch_cosine_similarity(
                anchor_embedding_tile,
                self.history_embeddings[negative_indexes],
            )

            # s23 = time()
            similar_negative_index = negative_indexes[
                np.argsort(anchor_cos)[-1]
            ]  # [-1:]
            ...
            anchor_cos = batch_cosine_similarity(
                anchor_embedding_tile,
                self.history_embeddings[positive_indexes],
            )
            dissimilar_positive_index = positive_indexes[
                np.argsort(anchor_cos)[0]
            ]  # [:1]
        ...
        anchor_speakers = [
            extract_speaker(a) for a in self.history_utterances[anchor_indexes]
        ]
        positive_speakers = [
            extract_speaker(a)
            for a in self.history_utterances[dissimilar_positive_indexes]
        ]
        negative_speakers = [
            extract_speaker(a)
            for a in self.history_utterances[similar_negative_indexes]
        ]
        ...
        assert list(
            self.history_utterances[dissimilar_positive_indexes]
        ) != list(self.history_utterances[anchor_indexes])
    ...
    def get_speaker_verification_data(
        self, anchor_speaker, num_different_speakers
    ):
        speakers = list(self.audio.speakers_to_utterances.keys())
        anchor_utterances = []
        positive_utterances = []
        ...
        assert [
            negative_speaker != anchor_speaker
            for negative_speaker in negative_speakers
        ]
        pos_utterances = np.random.choice(
            self.sp_to_utt_test[anchor_speaker], 2, replace=False
        )
        neg_utterances = [
            np.random.choice(self.sp_to_utt_test[neg], 1, replace=True)[0]
            for neg in negative_speakers
        ]
        anchor_utterances.append(pos_utterances[0])
        positive_utterances.append(pos_utterances[1])
...
class TripletBatcher:
    def __init__(self, kx_train, ky_train, kx_test, ky_test):
        ...
        assert speakers_list == sorted(
            set(ky_test.argmax(axis=1))
        ...
        assert sorted(
            sum([v for v in self.train_indices_per_speaker.values()], [])
        ) == sorted(range(len(ky_train)))
        assert sorted(
            sum([v for v in self.test_indices_per_speaker.values()], [])
    ...
    def select_speaker_data(self, speaker, n, is_test):
        ...
        indices_per_speaker = (
            self.test_indices_per_speaker
            if is_test
            else self.train_indices_per_speaker
        )
...
class TripletBatcherMiner(TripletBatcher):
    def __init__(
        self, kx_train, ky_train, kx_test, ky_test, model: DeepSpeakerModel
    ):
...
class TripletBatcherSelectHardNegatives(TripletBatcher):
    ...
    def get_batch(self, batch_size, is_test=False, predict=None):
        ...
        assert (
            len(set(selected_negative_speakers).intersection(anchor_speakers))
            == 0
        )
...
class TripletEvaluator:
    ...
    def get_speaker_verification_data(
        self, positive_speaker, num_different_speakers
    ):
